...
export async function deleteWeight(data: FormData) {
  const supabase = (await createClient()); await requireUser(supabase);
...
export async function saveMeasurement(data: FormData) {
  ...
  const input = parsed.data!;
  const supabase = (await createClient()); const user = await requireUser(supabase); const units = formValue(data, 'unit_system');
...
export async function deleteProgressPhoto(data: FormData) {
  const supabase = (await createClient()); await requireUser(supabase); const id = formValue(data, 'id');
