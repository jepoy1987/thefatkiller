...
export async function saveCoachGoal(data:FormData) {
  ...
  const input=parsed.data!; const supabase=(await createClient()); await requireUser(supabase);
...
async function setGoalStatus(data:FormData,status:'completed'|'archived') {
  const clientId=formValue(data,'client_id'); const path=`/coach/clients/${clientId}`; const supabase=(await createClient()); await requireUser(supabase);
