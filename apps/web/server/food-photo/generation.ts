...
export async function runFoodPhoto(image:Uint8Array,ports:PhotoPorts):Promise<string>{
 ...
 try{
  ...
  const parsed=foodPhotoResultSchema.safeParse(await ports.provider(image));if(!parsed.success||!isSafeFoodPhotoResult(parsed.data))throw new FoodPhotoError('invalid_output');result=parsed.data;
