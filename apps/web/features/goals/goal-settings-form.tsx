...
export function GoalSettingsForm({ profileUnits, goal }: Props) {
  ...
  return <form action={updateGoalSettings} className="grid gap-6"><section className="grid gap-5"><h3 className="text-sm font-bold uppercase tracking-[0.14em] text-muted-foreground">Plan basics</h3><div className="grid gap-5 sm:grid-cols-2">
    ...
    <FormField id="goals-goal-weight" label={`Goal weight (${display.units === 'imperial' ? 'lb' : 'kg'})`}><Input id="goals-goal-weight" name="goal_weight" type="number" inputMode="decimal" min="0.01" step="0.1" value={display.goalWeight} onChange={(event) => setDisplay((current) => ({ ...current, goalWeight: Number(event.target.value) }))} /></FormField>
  ...
  </div></section><section className="grid gap-5 border-t pt-6"><div><h3 className="text-sm font-bold uppercase tracking-[0.14em] text-muted-foreground">Daily targets</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">These are planning inputs shown on Today.</p></div><div className="grid gap-5 sm:grid-cols-2">{numericFields.map((field) => <FormField key={field.name} id={`goals-${field.name}`} label={field.label}><Input id={`goals-${field.name}`} name={field.name} type="number" inputMode={field.integer ? 'numeric' : 'decimal'} min={field.allowZero ? 0 : 0.01} step={field.integer ? 1 : 0.1} defaultValue={field.current} /></FormField>)}
    <FormField id="goals-daily-water-target" label={`Water (${display.units === 'imperial' ? 'fl oz' : 'ml'})`}><Input id="goals-daily-water-target" name="daily_water_target" type="number" inputMode="decimal" min="0.01" step="0.1" value={display.water} onChange={(event) => setDisplay((current) => ({ ...current, water: Number(event.target.value) }))} /></FormField></div></section>
