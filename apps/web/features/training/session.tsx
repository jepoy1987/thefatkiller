...
function SetEditor({ exercise, log, number, units, dirty, saved }: { exercise: WorkoutSessionExercise; log?: WorkoutSetLog; number: number; units: UnitSystem; dirty: () => void; saved: () => void }) {
  ...
  // Keep unrelated refreshes from remounting this form; clear only its own saved draft.
  const [saveVersion, setSaveVersion] = useState(0);
  const onSaved = () => { saved(); if (!log) setSaveVersion(version => version + 1); };
  ...
  return <div className="rounded-lg border p-3"><ActionForm key={saveVersion} action={saveWorkoutSet} label={log ? `Save set ${number}` : `Save new set ${number}`} onDirty={dirty} onClean={saved} onSaved={onSaved}>
