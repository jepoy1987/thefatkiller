...
import { numericInputProps } from '../goals/numeric-input';
...
export function OnboardingForm({ profile }: { profile: Profile }) {
  ...
  return <form action={completeOnboarding} className="mt-8 grid gap-5"><ol aria-label="Setup progress" className="grid grid-cols-4 gap-2">{['About you', 'Goal', 'Targets', 'Review'].map((label, index) => <li key={label} className="min-w-0"><span className="block h-1.5 rounded-full bg-primary" /><span className="mt-2 hidden truncate text-xs font-semibold text-muted-foreground sm:block">{index + 1}. {label}</span></li>)}</ol>
    ...
    <OnboardingStep number={2} title="Your Goal">
      ...
      <div className="grid gap-5 sm:grid-cols-3"><FormField id="starting-weight" label={`Starting weight (${weightUnit})`}><Input id="starting-weight" name="starting_weight" required type="number" {...numericInputProps(false, false)} /></FormField><FormField id="goal-weight" label={`Goal weight (${weightUnit})`}><Input id="goal-weight" name="goal_weight" required type="number" {...numericInputProps(false, false)} /></FormField><FormField id="height" label={`Height (${heightUnit})`}><Input id="height" name="height" required type="number" {...numericInputProps(false, false)} /></FormField></div>
