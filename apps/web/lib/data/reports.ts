...
export async function getReport(query:Record<string,string|undefined>,clientId?:string) {
 ...
 if(result.error){if(result.error.code==='42501')notFound();if(result.error.code==='54000')return {allowed:true,report:null,error:'Reports are being loaded too often. Wait a minute, then try again.',context};throw new Error('Report data could not be loaded.');}
