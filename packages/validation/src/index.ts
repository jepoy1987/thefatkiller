...
export const exerciseCategories = ['strength','cardio','mobility','conditioning','core','other'] as const;
export const exerciseEquipment = ['none','barbell','dumbbell','kettlebell','machine','cable','band','bodyweight','cardio_machine','other'] as const;
export const exerciseTrackingTypes = ['sets_reps','duration','distance','duration_distance','bodyweight','other'] as const;
const optionalNumber=(max:number,integer=false)=>z.preprocess(v=>v===''||v==null?null:v,(integer?z.coerce.number().int():z.coerce.number()).positive().max(max).nullable());
const text=(max:number)=>z.string().trim().max(max).optional();
export const exerciseSchema=z.object({id:z.string().uuid().optional(),name:z.string().trim().min(2).max(120),description:text(1000),category:z.enum(exerciseCategories),equipment:z.enum(exerciseEquipment),tracking_type:z.enum(exerciseTrackingTypes),instructions:text(2000)}).strict();
export const workoutTemplateItemSchema=z.object({exercise_id:z.string().uuid(),target_sets:optionalNumber(100,true),target_reps_min:optionalNumber(10000,true),target_reps_max:optionalNumber(10000,true),target_duration_seconds:optionalNumber(86400,true),target_distance_meters:optionalNumber(1000000),target_rest_seconds:z.preprocess(v=>v===''||v==null?null:v,z.coerce.number().int().min(0).max(3600).nullable()),notes:text(1000)}).strict().refine(v=>v.target_reps_min==null||v.target_reps_max==null||v.target_reps_max>=v.target_reps_min,{message:'Maximum reps must be at least minimum reps.',path:['target_reps_max']});
export const workoutTemplateSchema=z.object({id:z.string().uuid().optional(),name:z.string().trim().min(2).max(120),description:text(1000),items:z.array(workoutTemplateItemSchema).min(1).max(30)}).strict();
